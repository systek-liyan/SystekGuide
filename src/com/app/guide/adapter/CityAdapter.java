...
public class CityAdapter extends CommonAdapter<CityModel> {
	...
	public CityAdapter(Context context, List<CityModel> data, int layoutId) {
		super(context, data, layoutId);
		alphaIndexer = new HashMap<String, Integer>();
		// 匹配每一个CityModel的NameSort,如果不等于前一个CityModel的NameSort，证明该城市是某一字母的第一个城市，
		// 则将该CityModel的position加入到Map中
		for (int i = 0; i < mData.size(); i++) {
			String currentStr = mData.get(i).getNameSort();
			String previewStr = (i - 1) >= 0 ? mData.get(i - 1).getNameSort()
					: " ";
			if (!previewStr.equals(currentStr)) {
				String name = mData.get(i).getNameSort();
	...
	public void convert(ViewHolder holder, int position) {
		holder.setTvText(R.id.item_city_name, getItem(position).getCityName());
		TextView alpha = holder.getView(R.id.item_city_alpha);
		String currentStr = getItem(position).getNameSort();
